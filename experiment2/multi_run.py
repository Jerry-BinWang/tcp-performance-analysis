...
latency_list = ['1ms']
tcp_pair_list = [('Agent/TCP/Vegas', 'Agent/TCP/Vegas'),
                 ('Agent/TCP/Newreno', 'Agent/TCP/Vegas')]
cbr_packet_size_list = [1000]
cbr_bandwidth_list = ['5mb']
start_time_diff_list = [-1]
tcp_run_time = 20

def run_ns2_simulation(latency, tcp_1, tcp_2, cbr_packet_size, cbr_bandwidth, start_time_diff, i):
    if start_time_diff < 0:
        cbr_start_time = 0
        tcp_start_time = - start_time_diff + random.uniform(0.0, 1.0)
        print(tcp_start_time)
    else:
        tcp_start_time = 0
            ...
            for cbr_packet_size in cbr_packet_size_list:
                for cbr_bandwidth in cbr_bandwidth_list:
                    for start_time_diff in start_time_diff_list:
                        for i in range(100):
                            run_ns2_simulation(latency, tcp_1, tcp_2, cbr_packet_size, cbr_bandwidth, start_time_diff, i)